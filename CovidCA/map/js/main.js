...
function createMap(){
    ...
    //create the map
    var map = L.map('map', {
        center: [46, 67.20],//Centered to Central Asia
        zoom: 5,
        zoomControl: false,
        layers:grayscale
    });
...
function getData(map){
    ...
    $.ajax("data/CentralAsia_PointFinal.geojson", {
        dataType: "json",
...
function processPolyData(data) {
    ...
    //properties of the first feature in the dataset
    var properties = data.feature[0].properties;
    
        //push each attribute name into attributes array
    for (var attribute in properties) {
        //only take attributes with population values
        if (attribute.indexOf("UNIT_NAME") > -1) {
