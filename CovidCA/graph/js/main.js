(function () {
    ...
    var attrArray = ['Cases', 'Deaths', 'Recovered', 'Population']
    var expressed = attrArray[0] //initial attribute is Cases

    console.log(attrArray[1])//Deaths
    console.log(attrArray[2])//Recovered
    console.log(attrArray[3])//Population
    ...
    //Get Max Value from CSV 
    function getMaxByProperty(csvData, propName) {
        if (!csvData) return 0
        var temp = csvData.map(function (item) {
            return parseInt(item[propName], 10)
        })

        var max = Math.max.apply(null, temp)
        return max + Math.pow(10, max.toString(10).length - 1)
    }
    //Change the Y Scale Domain Max to update when data is selected
    function yScaleFunc(expressed, csvData) {

        switch (expressed) {
            case 'Deaths': {
                yScale.domain([0, (getMaxByProperty(csvData, 'Deaths'))])
                break
            }
            case 'Recovered': {
                yScale.domain([0, (getMaxByProperty(csvData, 'Recovered'))])
                break
            }
            case 'Population': {
                yScale.domain([0, (getMaxByProperty(csvData, 'Population'))])
                break
            }
            default: {
                yScale.domain([0, (getMaxByProperty(csvData, 'Cases'))])
                break
            }
        }
    }
    ...
    function updateChart(bars, n, colorScale, csvData, chart) {
        ...
        var chartTitle = d3.select('.chartTitle')
            if (expressed == "Cases"){
                chartTitle.text ('The Total Number of Positive Covid-19 Cases')} 
            else if (expressed == "Deaths"){
                chartTitle.text ('The Total Number of Deaths Related to Covid-19')} 
            else if (expressed == "Recovered"){
                chartTitle.text ('The Total Number of People who have Recovered from Covid-19')}
            else if (expressed == "Population")
                chartTitle.text ('Total Population from 2015')
    };
    ...
    function setLabel(props) {
        //label content
        var labelAttribute = '<h1>' + props[expressed] + ' ' + expressed + '<br>' + '</h1>'
